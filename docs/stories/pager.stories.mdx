...
| staticMode  | 静态分页模式（wcm） | Boolean | --       | false  |
...
| 参数             | 说明                         | 类型 | 可选值 | 默认值 |
| ---------------- | ---------------------------- | ---- | ------ | ------ |
| isXDomainRequest | 特殊参数，ie8,9 中请求可跨域 | --   | --     | false  |
...
return {
    ...
    totalNumber: res.totalNumber //总条数,
};
```
...
| complete | 请求完成后回调   | Function | null   | --       |

|sendBefore | 请求前回调 | Function | null | setParams:请求前设置参数方法 |
...
| 方法名  | 说明               | 类型     | 默认值 | 参数     | 备注                                            |
| ------- | ------------------ | -------- | ------ | -------- | ----------------------------------------------- |
| jump    | 外部跳转方法       | Function | --     | 页码     | pageNo ,callback                                |
| set     | 参数外部设置       | Function | --     | 参数集合 | obj.set({url:'xxx',data:{keyword:22}}),callback |
| setJump | 参数外部设置并跳转 | Function | --     | 参数集合 | obj.set({url:'xxx',data:{keyword:22}}),callback |
