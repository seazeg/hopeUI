...
<Canvas>
    <Story name="通用" height="300px" parameters={{ docs: { source: { code:`${viewinfo.general}${viewinfo.generalJS}` }}}}>
    {() => {
        useEffect(() => {
...
<Canvas>
    <Story name="分组" height="300px" parameters={{ docs: { source: { code:`${viewinfo.group}${viewinfo.groupJS}` }}}}>
    {() => {
        useEffect(() => {
