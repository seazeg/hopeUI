...
<Canvas>
    <Story
        ...
        height="300px"
        parameters={{
            docs: { source: { code: `${viewinfo.group}\n\n<script type="text/javascript">\n${viewinfo.groupJS}\n</script>` } },
        }}
    >
        {() => {
            useEffect(() => {
...
| isTitle       | 下拉选项是否含有 title 属性 | Boolean | --     | false  |
| placeholder   | 默认显示内容                | String  | --     | 请选择 |
| hideItemIndex | 隐藏某一项                  | Number  | --     | 0      |
...
| set    | 设置选中项   | Function | --     | type 值(value/label),value 值,callback |
| update | 内容更新同步 | Function | --     | callback                               |
