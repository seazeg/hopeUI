...
var hopeu = (window["hopeu"] = (function () {
                  ...
                  if (nt !== 3 && nt !== 8 && nt !== 2) {
                      if (isO(name))
                          for (var k in name)
                              ...
                              else this.setAttribute(k, name[k]);
                      else
                          this.setAttribute(
                              name,
                              isF(val)
    ...
    p["addClass"] = function (val) {
        ...
        if (val && isS(val)) {
            ...
            for (i = 0, l = this.length; i < l; i++) {
                el = this[i];
                if (el && el.nodeType === 1) {
                    if (!el.className && cls.length === 1) el.className = val;
                    else {
    ...
    function make(arr, els) {
        ...
        for (var i = 0, l = els.length; i < l; i++) arr[i] = els[i];
        return arr;
    }
    ...
    function hasClass(els, cls) {
        ...
        for (var i = 0, l = els.length; i < l; i++)
            if (eqClass(els[i], cls)) return true;
        return false;
    }
    $["hasClass"] = hasClass;
    ...
    function eqClass(el, cls) {
        return (
            el.nodeType === 1 &&
            (" " + el.className + " ").replace(rclass, " ").indexOf(cls) > -1
        );
    }
    ...
    function walk(fn, ctx, ret) {
        ctx = ctx || doc;
        ret = ret || [];
        if (ctx.nodeType == 1) if (fn(ctx)) ret.push(ctx);
        var els = ctx.childNodes;
        for (var i = 0, l = els.length; i < l; i++) {
            var subEl = els[i];
            if (subEl.nodeType == 1) walk(fn, subEl, ret);
        }
        return ret;
    }
    $["walk"] = walk;
    ...
    $["each"] = function (o, cb, args) {
        var k,
            i = 0,
            l = o.length,
            isObj = l === undefined || isF(o);
        if (args) {
            if (isObj) {
                for (k in o) if (cb.apply(o[k], args) === false) break;
            } else for (; i < l; ) if (cb.apply(o[i++], args) === false) break;
        } else {
            if (isObj) {
                for (k in o) if (cb.call(o[k], k, o[k]) === false) break;
            } else
                for (; i < l; ) if (cb.call(o[i], i, o[i++]) === false) break;
        }
        return o;
    };
    ...
    function _each(o, fn, ctx) {
        if (o == null) return;
        if (nativeForEach && o.forEach === nativeForEach) o.forEach(fn, ctx);
    ...
    $["_defaults"] = function (obj) {
        _each(slice.call(arguments, 1), function (o) {
            for (var k in o) if (obj[k] == null) obj[k] = o[k];
        });
        return obj;
    };
    ...
    function dir(el, prop, until) {
        ...
        while (
            cur &&
            cur.nodeType !== 9 &&
            (until === undefined || cur.nodeType !== 1 || !$(cur).is(until))
        ) {
    ...
    trim = strim
        ? function (text) {
              return text == null ? "" : strim.call(text);
          }
        : function (text) {
              return text == null
                  ? ""
    ...
    function isF(o) {
        return typeof o == "function" || typeOf(o) === "function";
    }
    $["isFunction"] = isF;
    ...
    function isAL(o) {
        return !isS(o) && typeof o.length == "number";
    }
    ...
    function isPlainObj(o) {
        if (!o || typeOf(o) !== "object" || o.nodeType || isWin(o))
            return false;
        try {
            if (
                o.constructor &&
    ...
    function merge(a1, a2) {
        ...
        else while (a2[j] !== undefined) a1[i++] = a2[j++];
        a1.length = i;
        return a1;
    }
    $["merge"] = merge;
            ...
            if ((opt = args[i]) != null) {
                for (name in opt) {
                    ...
                    if (
                        ...
                        } else clone = src && isPlainObj(src) ? src : {};
                        dst[name] = extend(deep, clone, copy);
                    } else if (copy !== undefined) dst[name] = copy;
                }
            }
        }
        return dst;
    ...
    function htmlFrag(html, ctx, frag) {
        ...
        if (isAL(html)) return clean(html, ctx, frag) && frag;
    ...
    function clean(els, ctx, frag) {
        ...
        for (i = 0; (el = els[i]) != null; i++) {
            if (isS(el)) el = fragDiv(el, ctx);
            if (el.nodeType) ret.push(el);
            else ret = merge(ret, el);
        }
        if (frag)
            for (i = 0; i < ret.length; i++)
                if (ret[i].nodeType) frag.appendChild(ret[i]);
        return ret;
    }
    ...
    if (doc["body"] && !$["init"]) setTimeout($["onload"], 1); //let plugins loadAsync
...
hopeu["plug"]("css", function ($) {
    ...
    $["_each"](["height", "width"], function (k) {
        $["cssHooks"][k] = {
            get: function (el, comp, extra) {
                var val;
                if (comp) {
                    if (el.offsetWidth !== 0) return getWH(el, k, extra);
            ...
            if (extra !== "border") {
                $["each"](which, function () {
                    ...
                    if (extra === "margin")
                        ...
                        val -=
                            parseFloat(css(el, "border" + this + "Width")) || 0;
                });
            }
            return val + "px";
        }
    ...
    function style(el, name, value, extra) {
        ...
        if (value !== undefined) {
            ...
            if (
                !hooks ||
                !("set" in hooks) ||
                (value = hooks.set(el, value)) !== undefined
            ) {
                try {
                    style[name] = value;
                } catch (e) {}
            ...
            if (
                hooks &&
                "get" in hooks &&
                (ret = hooks.get(el, false, extra)) !== undefined
            )
                return ret;
            return style[name];
        }
    ...
    function css(el, name, extra) {
        ...
        if (
            hooks &&
            "get" in hooks &&
            (ret = hooks.get(el, true, extra)) !== undefined
        )
            return ret;
    ...
    var init,
        noMarginBodyOff,
        subBorderForOverflow,
        suppFixedPos,
        noAddBorder,
        noAddBorderForTables,
        initialize = function () {
            if (init) return;
            var body = doc.body,
                ...
                table,
                td,
                bodyMarginTop = parseFloat(css(body, "marginTop")) || 0,
                html =
    ...
    $["fn"]["position"] = function () {
        ...
        return {
            top: off.top - parOff.top,
            left: off.left - parOff.left,
        };
    };
    ...
    $["_each"](["Height", "Width"], function (name, i) {
        ...
        $["fn"]["outer" + name] = function (margin) {
            var el = this[0];
            return el && el.style
                ? parseFloat(css(el, type, margin ? "margin" : "border"))
                : null;
        };
        $["fn"][type] = function (size) {
            ...
            if ($["isWindow"](el)) {
                var docElProp = el.document.documentElement["client" + name],
                    body = el.document.body;
                return (
                    (el.document.compatMode === "CSS1Compat" && docElProp) ||
        ...
        return this["each"](function (i, el) {
            addEvt(el, evt, cb, sel, function (e) {
                ...
                if (target && !(target === el) && !(target === document)) {
                    cb.call(
                        target,
                        $["extend"](createProxy(e || window.event), {
...
hopeu["plug"]("ajax", function ($) {
    ...
    function _xhrResp(xhr, dataType) {
        ...
        if (dataType.indexOf("script") >= 0) return eval(xhr.responseText);
        if (dataType.indexOf("xml") >= 0) return xhr.responseXML;
        ...
        for (var k in o)
            kvps.push(
                encodeURIComponent(k).replace(regEx, "+") +
                    "=" +
                    encodeURIComponent(o[k].toString()).replace(regEx, "+")
            );
        return kvps.join("&");
    };
