var Hope_carousel = function (selector, params, index) {
    ...
    if (!document.querySelectorAll) {
        if (!window.hopeu) return;
    }

    function $$(selector, context) {
        if (document.querySelectorAll)
            return (context || document).querySelectorAll(selector);
        else return hopeu(selector, context);
    }
    ...
    _this._extendHopeSlide = function (el) {
        ...
        if (!el.hopeSlideDataStorage) el.hopeSlideDataStorage = {};
        el.getData = function (name) {
            return el.hopeSlideDataStorage[name];
        };
        el.setData = function (name, value) {
            el.hopeSlideDataStorage[name] = value;
            return el;
        };
    ...
    function handleKeyboardKeys(e) {
        ...
        if (kc === 37 || kc === 39 || kc === 38 || kc === 40) {
            var inView = false;
            var hopeOffset = _this.h.getOffset(_this.container);
            ...
            var hopeCoord = [
                ...
                [hopeOffset.left + _this.width, hopeOffset.top + _this.height],
            ];
            for (var i = 0; i < hopeCoord.length; i++) {
    ...
    _this.reloadSlides = function () {
        ...
        var firstIndex = Math.max(
            0,
            newActiveIndex - params.slidesPerView * params.loader.surroundGroups
        );
...
Hope_carousel.prototype.plugins.effect = function (Hope_carousel, params) {
    var isH = Hope_carousel.params.mode == "horizontal";
    ...
    function initSlides() {
        ...
        for (var i = 0; i < Hope_carousel.slides.length; i++) {
            ...
            if ("offsetLeft" in slide) {
                slide.effectSlideOffset = isH
                    ? slide.offsetLeft
                    : slide.offsetTop;
            } else {
                slide.effectSlideOffset = isH
                    ? slide.getOffset().left -
                      Hope_carousel.h.getOffset(Hope_carousel.container).left
                    : slide.getOffset().top -
                      Hope_carousel.h.getOffset(Hope_carousel.container).top;
            }
        }
        if (isH) {
            wrapperMaxPosition =
    ...
    function calcEffect(transform) {
        var transform = transform || { x: 0, y: 0, z: 0 };
        var offsetCenter;
        if (Hope_carousel.params.centeredSlides == true)
            offsetCenter = isH
        ...
        for (var i = 0; i < Hope_carousel.slides.length; i++) {
            var slide = Hope_carousel.slides[i];
            var slideCenterOffset =
                Hope_carousel.params.centeredSlides == true
                    ? slide.effectSlideSize / 2
                    : 0;

            var offsetMultiplier =
                (offsetCenter - slide.effectSlideOffset - slideCenterOffset) /
                slide.effectSlideSize;
            slide.effect = offsetMultiplier;
        }
        // Global Hope_carousel Effect
        Hope_carousel.effect = isH
            ? -transform.x / wrapperMaxPosition
            : -transform.y / wrapperMaxPosition;
        // Callback
        if (Hope_carousel.params.onEffectChange)
    ...
    var hooks = {
        onFirstInit: function (args) {
            // for (var i = 0; i < Hope_carousel.slides.length; i++) {
            //     let temp = Hope_carousel.slides[i].innerHTML;
            //     Hope_carousel.slides[i].innerHTML = `<div class="flip-container">${temp}</div>`
            // }
